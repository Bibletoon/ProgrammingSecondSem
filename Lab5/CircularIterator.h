...
public:
	...
	CircularIterator() : _pointer(nullptr), _capacity(0), _dataBegin(nullptr), _dataEnd(nullptr) {}
	CircularIterator(T* pointer, T* dataBegin, T* dataEnd, size_t capacity) : _pointer(pointer), _dataBegin(dataBegin), _dataEnd(dataEnd), _capacity(capacity) {}
	CircularIterator(const CircularIterator& iterator) : _pointer(iterator._pointer), _dataBegin(iterator._dataBegin), _dataEnd(iterator._dataEnd), _capacity(iterator._capacity) {}
	...
	{
		return CircularIterator(_dataBegin + ((_pointer - _dataBegin + rhs) % _capacity),_dataBegin,_dataEnd,_capacity);
	}
	CircularIterator operator-(int rhs) const
	{
